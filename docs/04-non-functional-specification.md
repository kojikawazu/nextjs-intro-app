...
| `<section>` | Hero, About, Career, AI, Product, Articles, Contact 各セクション。`CareerCard` 内の技術スタック / 担当フェーズ / 役割にも使う |
| `<article>` | 経歴カード (`CareerCard`)・個人開発カード (`ProductCard`)・記事の行 (`ArticleEntry`)・AI 活用の方針の行 (`AiPracticeEntry`)。1 件が独立して意味を持つ内容のため |
| `<footer>` | ページフッター (コピーライト) |
| `<h1>` | ヒーロー見出し（ページ内に 1 つ）。**ヘッダーロゴは `<span>`**。見出しの階層を持たない装飾的なラベルであり、`<h1>` が 2 つになるのを避けるため |
| `<h2>` | 各セクションタイトル |
| `<h3>` | 経歴カード・個人開発カードのタイトル、記事タイトル、AI 活用の方針の見出し、送信完了の見出し |
