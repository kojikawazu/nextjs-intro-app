...
## 1.3 React 19 における位置づけ

React 19 では **`ref` を通常の props として関数コンポーネントへ直接渡せる**ようになり、`forwardRef` は「必須」ではなくなった。ただし**非推奨化・削除はされておらず互換性は維持**される。

本プロジェクトは issue #104 で React 19.3.0 へ移行したが、**`forwardRef` の実装はそのまま維持している**。

| 観点 | 判断 |
|---|---|
| 動作 | 実ブラウザで検証済み。react-hook-form の `register()` が返す ref が透過し、Zod 検証・`aria-invalid` / `aria-describedby` の付与まで正常に動作する |
| 警告 | コンソールに `forwardRef` 関連の非推奨警告は**出ない** |
| 書き換えの是非 | ref を props 化する書き換えは React 19 以降でしか動かなくなる。動作している実装を、利得の乏しい理由で触る必要はない |

将来 `forwardRef` が実際に非推奨化された段階で、本ドキュメントとあわせて書き換えを検討する。
