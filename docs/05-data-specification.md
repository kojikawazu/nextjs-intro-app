...
- [5. データ形式仕様](#5-データ形式仕様)
    ...
    - [5.4 表示位置が特殊なフィールド](#54-表示位置が特殊なフィールド)
        - [about_contents1 は Hero に表示する](#about_contents1-は-hero-に表示する)
        - [画面から参照されないフィールド](#画面から参照されないフィールド)
    - [5.5 技術スタックの分類](#55-技術スタックの分類)
        - [分類の定義](#分類の定義)
        - [運用上の注意: 新技術は黙って「その他」へ落ちる](#運用上の注意-新技術は黙ってその他へ落ちる)
    - [5.6 メール送信日時形式](#56-メール送信日時形式)
- [6. 環境変数一覧](#6-環境変数一覧)
...
| フィールド名 | 型 | 必須 | 説明 | 例 |
|---|---|---|---|---|
| `about_name` | `string` | Yes | 表示名 / 氏名。**画面には文字として出さず、プロフィール画像の `alt` に使う**（issue #138） | `"Koji Kawazu"` |
...
| `about_contents` | `string[]` | Yes | 自己紹介文（段落ごとの配列）。**`[1]` だけは About ではなく Hero に表示する**（後述） | `["段落1のテキスト", "段落2のテキスト"]` |
...
| `sns_img` | `string` | Yes | SNSアイコン画像のURL。**現在の UI では未参照**（後述） | `"https://storage.googleapis.com/.../github.svg"` |
...
| フィールド | 形式 | 備考 |
|---|---|---|
| `about_contents` | プレーンテキスト配列 | 各要素が1段落として表示される（`[1]` のみ Hero へ） |
| `career_contents` | プレーンテキスト | 改行なしの単一文字列 |
| `copyright` | プレーンテキスト | HTMLエンティティは使用しない |

### 5.4 表示位置が特殊なフィールド

#### `about_contents[1]` は Hero に表示する

デザイン刷新（issue #135 / #138）で、`about_contents` の **2 番目の段落だけを About から Hero 直下へ移動**した。About にはそれ以外の段落が入る。

Hero の見出しは `Solving Problems with Technology`（コードの固定文字列）で、「何ができる人か」を述べていない。書体と余白では解けないが、`about_contents[1]` がすでにその答えを書いていた。

> 専門はバックエンド開発。新規開発から既存システムの機能変更や保守、技術調査検証など幅広く担当しており、特に要件定義(検討)から設計、実装、テスト設計、レビューまでの一連の流れを経験しております。

**新規コピーを書かず、既存データの再配置で解いている。** データ側の文言・並び順は変更していないため、`[1]` に別の内容を入れると Hero の意味が変わる点に注意する。

#### 画面から参照されないフィールド

以下は JSON に存在するが、現在の UI では読まれない。

| フィールド | 理由 |
|---|---|
| `hero_data.hero_img_url` | Hero の背景画像を廃止したため（issue #138） |
| `about_data.about_icon_url` | 使用箇所が無い |
| `about_data.sns_list[].sns_img` | 指す SVG が白一色で、明るい地の上では見えないため、`sns_name` のテキストチップに置き換えた（issue #138）。CSS フィルタで反転させる手もあるが、将来データ側が色付きアイコンに差し替わると破綻する |
| `career_title_data` | ラベルをコード側でハードコードしているため（§2.6） |
| `contact_data` | フォームのラベルをコード側でハードコードしているため |

**削除はしない。** データ側を変更しない方針（issue #135）であり、将来の参照再開もありうるため。
