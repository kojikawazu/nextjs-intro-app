...
| 日付 | 内容 | 担当 |
|------|------|------|
| 2026-09-25 | 要件定義（docs/02）とスコープ（docs/01）を現在の実装・掲載データへ同期（issue #169）。#138 などで挙動が変わった後も旧実装を要件として書いていた FR を改め、廃止した FR（固定ヘッダー・スクロール時の背景切替・Hero の背景画像と CTA）は欠番として理由を残した。AI / Product / Articles セクション・テーマ切替・送信回数の制限・正規 URL の FR を追加。docs/02 §4・§5（技術スタック・構成・API・環境変数）は docs/07・09 の古い写しになっていたため、正本への参照に置き換えた。docs/01 のスコープ表に残っていた Skills セクションと「ダーク系で固定」も是正 | - |
| 2026-09-25 | 経歴表示の要件 FR-CAREER-001 を「タイムライン表示（縦ライン+ドット）」から「経歴の時系列提示」へ改めた（issue #149）。縦線・ドットは要件ではなく当時の実装手段だったと判断し、#138 で廃した実装に戻さない。**並び順は `career_data` の配列順が正**で、画面側では並べ替えないことを明記。docs/01 §3.1 の Career 行も同様に改めた。同じ種類のずれ（旧実装を要件として書いた FR）が docs/02・01 に多数あったため、issue #169 へ切り出した | - |
| 2026-09-25 | issue #148 から引き継いだ未実装の E2E 計画行の採否を決め、採用分を実装（issue #166）。ナビの出し分け（`md:flex` / `md:hidden`、767/768px の境界を含む）とキーボード操作・フォーカス表示・`<html lang="ja">` の 9 ケースを追加（E2E 47→56）。About / Career への移動は同じ経路を既存 3 本が確認済み、各幅での「崩れない」は判定手段が無いため不採用とし、理由を docs/08 の欠番注記に残した。フォーカス表示の検査は Tailwind 3 の `outline-none` が透明な outline を残す点を踏まえて色まで見る。わざと ring やブレークポイントを壊して 3 件とも検出できることを確認 | - |
| 2026-09-25 | docs/08 の E2E 仕様（§6.1〜§6.5）を実装済みのテストを正本として書き直し（issue #148）。E2E 導入前の計画表が残っており、ローディング表示・Hero の CTA・ヘッダーの半透明化・Skills のグリッドなど実在しない挙動や、テストが確認していない期待結果が載っていた。UT で担保済みのケースは参照に置き換え、どこにもテストが無いケースは issue #166 へ引き継いだ。削除した ID は欠番とし再利用しない。あわせて §1.1 のテスト件数（UT 33→354 / IT 10→18 / E2E 7→47）と「コンポーネントテストは未実装」、§9.1 の「E2E は未実装」を是正 | - |
